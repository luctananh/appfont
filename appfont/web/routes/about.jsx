// Import các component cần thiết từ thư viện Shopify Polaris và React
import {LegacyCard,Tabs,Button,Checkbox,TextField,DropZone,LegacyStack,Thumbnail,Layout,Page,Select,Card,Toast,Frame,Modal,Text,ButtonGroup,} from '@shopify/polaris';
...
export default function FontManager() {
    ...
    const [files, setFiles] = useState([]); // Mảng lưu trữ các file đã upload
    const [selectedElements, setSelectedElements] = useState({ // Object để theo dõi các element HTML được chọn để áp dụng font
        h1: false,
        h2: false,
        h3: false,
        h4: false,
    ...
    //     name: 'dev-app-font-test',
    //     domain: 'dev-app-font-test.myshopify.com',
    //     accessToken: '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'
    // }); // loại bỏ vì không sử dụng
    ...
    const handleTabChange = useCallback(
        ...
        [], // Mảng dependency rỗng để đảm bảo callback không bị tạo lại mỗi khi render
    );
    ...
    const fetchGoogleFontsFromApi = async () => {
        try {
            ...
            if (!response.ok) { // Kiểm tra xem request API có thành công không
   ...
   // Hàm upload file font tùy chỉnh lên Gadget API
    const uploadFileToGadget = async (file) => {
        ...
        try {
            ...
            reader.onloadend = async () => { // Event khi file đã đọc xong
                const base64Data = reader.result.split(',')[1]; // Lấy data base64
                try {
                    const existingFont = await api.datafontgg.findFirst({ // Kiểm tra xem font với keyfont=upload đã tồn tại chưa
                        filter: {
                    ...
                    let response;
                    if (existingFont) {
                        response = await api.datafontgg.update(existingFont.id, { // Cập nhật font đã có
                            datafontgg: {
                                name: fileName,
                                link: base64Data,
                            },
                        ...
                        response = await api.datafontgg.create({ // Tạo font mới
                            name: fileName,
                            link: base64Data,
                            keyfont: "upload",
                        });
            ...
            reader.onerror = (error) => { // Event khi đọc file bị lỗi
   ...
   // Hàm xử lý khi file được thả vào DropZone
    const handleDropZoneDrop = useCallback((_dropFiles, acceptedFiles) => {
        setFiles((files) => [...files, ...acceptedFiles]); // Thêm file đã thả vào state
    }, []);
     ...
     // Hàm xử lý khi checkbox tất cả các element thay đổi
       const handleAllElementsChange = useCallback(() => {
            setAllElementsSelected((prev) => {
              ...
              setSelectedElements({ // Cập nhật trạng thái của tất cả các checkbox element khác
                h1: newState,
                h2: newState,
                h3: newState,
                h4: newState,
   ...
   // Hàm lưu font sau khi upload
    const handleSave = async () => {
          if (files.length === 0 || !fileName) { // Validation: Check file và tên
              setToastContent('Please add a file and enter a name.');
              setToastActive(true);
              return;
          }
         try {
              const file = files[0]; // Lấy file từ state
             await uploadFileToGadget(file); // Upload file lên Gadget API

            // Reset file và tên sau khi lưu thành công
            setFiles([]); // Reset state danh sách file
   ...
   // Hàm tạo/cập nhật cài đặt font đã chọn và lưu vào Gadget API
     const handleCreateUpdateSelectfont = async (keyfont, selectedFonts) => {
           try {
             setLoading(true); // Set trạng thái loading
             const shop = await api.shopifyShop.findFirst({ select: { id: true } }); // Lấy Shop ID từ API
             if (!shop || !shop.id) {
                 throw new Error('Could not fetch Shop ID');
             }
             const shopid = String(shop.id); // Lấy Shop ID dưới dạng string
             let selectedFontDetail; // Biến lưu trữ chi tiết font đã chọn
             if (keyfont === "upload") { // Xử lý font custom đã upload
                 ...
                 selectedFontDetail = latestFont[0]; // Lấy bản ghi font upload mới nhất
             } else { // Xử lý font Google
                const fontNamesArray = selectedFonts.split(',');
                 const fontDetails = googleFonts.filter(font => fontNamesArray.includes(font.value)) // Get detailed font data from googleFonts
                 if (!fontDetails || fontDetails.length === 0) {
                     setToastContent("Please select a font first.");
                     setToastActive(true);
                     setLoading(false)
                     return;
                 }
                selectedFontDetail = { // Create new object for selectedFontDetail
                      id: fontDetails[0].value,
                      name: fontDetails[0].label,
                      link: `https://fonts.googleapis.com/css2?family=${fontDetails[0].label.replace(/ /g, '+')}&display=swap`, // link css google font
                     keyfont: keyfont
                 }
             }
            // Tạo một chuỗi các tag đã chọn
             const selectedTags = Object.keys(selectedElements) // Lọc và lấy các element đã chọn
                 .filter((key) => selectedElements[key])
                 .join(',');
             if (!selectedTags) {
                 setToastContent('Please select at least one element to apply font.');
                 setToastActive(true);
                 setLoading(false)
                 return;
             }
             console.log('Selected font details:', selectedFontDetail);
             console.log('Selected shopid:', shopid);

            // Tìm bản ghi cài đặt font đã chọn cho shop hiện tại
             ...
             let data;
             const value = { // Tạo object chứa chi tiết font và element đã chọn để lưu trữ
                 id: selectedFontDetail.id,
                 name: selectedFontDetail.name,
                 link: selectedFontDetail.link,
                 selectedElements: selectedTags, // Lưu các tag đã chọn vào đây
             };
            // Tạo/Cập nhật cài đặt font đã chọn
             if (selectfontRecords.length === 0) {
                 data = await api.selectfont.create({ // Tạo mới bản ghi trong Gadget API
                     selectfont: {
                         shopid: shopid,
                         namespace: 'setting',
                         key: 'style',
                 ...
                 console.log('New selectfont record created successfully', data);
             } else {
                 data = await api.selectfont.update(selectfontRecords[0].id, { // Cập nhật bản ghi đã có trong Gadget API
                     selectfont: {
                         value: value
                     },
                 });
                 console.log('Selectfont record updated successfully', data);
             }


             //Set các element đã chọn
            if (value && value.selectedElements) { // Set các checkbox dựa trên giá trị đã lưu
                 ...
                 setSelectedElements({ // Nếu không có bản ghi set checkbox về false
                     h1: false,
                     h2: false,
                     h3: false,
                     h4: false,
            ...
            setFontDetails(selectedFontDetail); //Set chi tiết cho font đang chọn hiện tại
            await api.update1(); // Gọi API update1
            setToastContent('Font and theme settings saved successfully!');
             setToastActive(true);
         } catch (error) {
    ...
    const handleFontSelect = async () => {
            ...
            if (!selectedFonts) {
                setToastContent('Please select at least one font.');
                setToastActive(true);
                 return;
             }
         };
    ...
    const handleModalOpen = useCallback(() => {
        ...
        if (selectedFont) {
            ...
            googleFonts.forEach((font) => {
                 if (selectedFontArray.includes(font.value)) { // Kiểm tra nếu font đang chọn có trong danh sách google font
    ...
    const loadFontsForPage = (page, fonts = googleFonts) => {
        ...
        const fontsToLoad = fonts.slice(start,end); // Lấy các font cho trang hiện tại
         fontsToLoad.forEach(font => {
            ...
            if (!loadedFonts.includes(font.value)){ // Kiểm tra font đã được load chưa
                                            ...
                                            <>
                                                <DropZone onDrop={handleDropZoneDrop}>
                                                    ...
                                                    <p>
                                                        ...
                                                        <a href="https://www.fontspring.com?refby=NitroApps" target="_blank" rel="noopener noreferrer">
                                                    ...
                                                    ,
                                                    <a href="https://webfonts.ffonts.net" target="_blank" rel="noopener noreferrer">
                                                       https://webfonts.ffonts.net
                                                    </a>
                                                    ,
                                                    <a href="https://fontsforweb.com" target="_blank" rel="noopener noreferrer">
                                                       https://fontsforweb.com
                                                    </a>
                                              <p>
                                                3.Convert file to woff:<a href="https://cloudconvert.com/otf-to-ttf" target="_blank" rel="noopener noreferrer">https://cloudconvert.com/otf-to-ttf</a></p>
                                                <TextField
                                                    label="Font Name"
                                                    value={fileName}
                                                    onChange={handleNameChange}
                                                />
                                              <p>The font name should be unique and simple text, no special characters.</p>
                                            </>
                                        )}
                                        {selected === 1 && (
                                              <div>
                                               <TextField value={fontNamesSelected || ''} onClick={handleModalOpen}  />
                                                    ...
                                                    {totalPages > 1 && (
                                                     <div style={{ display: 'flex', justifyContent: 'center', margin: '10px 0' }}>
                                                        <ButtonGroup>
                                                         ...
                                                         <Text variant="bodySm" as="span" fontWeight="bold">{currentPage}/{totalPages}</Text>
                                                     ...
                                                     <ButtonGroup >
                                                      <Button primary onClick={handleModalClose} >Cancel</Button>
                                                      <Button primary onClick={handleFontSelect} >Confirm</Button>
                                                     </ButtonGroup>
                                                    </Modal.Section>
                                                </Modal>
                        ...
                        <Card title="Select Elements to Apply Font">
                          ...
                          <p>Select elements to assign font to:</p>
                              <div style={{ display: 'flex', flexDirection: 'column' }}>
                                  <Checkbox label="All" checked={allElementsSelected} onChange={handleAllElementsChange} style={{ marginBottom: '5px', display: 'block' }}/>
                                  <Checkbox label="Headline 1 (h1 tags)" checked={selectedElements.h1} onChange={() => handleElementChange('h1')} style={{ marginBottom: '5px', display: 'block' }} />
                                  <Checkbox label="Headline 2 (h2 tags)" checked={selectedElements.h2} onChange={() => handleElementChange('h2')} style={{ marginBottom: '5px', display: 'block' }}/>
                                  <Checkbox label="Headline 3 (h3 tags)" checked={selectedElements.h3} onChange={() => handleElementChange('h3')} style={{ marginBottom: '5px', display: 'block' }} />
                                  <Checkbox label="Headline 4 (h4 tags)" checked={selectedElements.h4} onChange={() => handleElementChange('h4')} style={{ marginBottom: '5px', display: 'block' }} />
                                  <Checkbox label="Headline 5 (h5 tags)" checked={selectedElements.h5} onChange={() => handleElementChange('h5')} style={{ marginBottom: '5px', display: 'block' }}/>
                                  <Checkbox label="Headline 6 (h6 tags)" checked={selectedElements.h6} onChange={() => handleElementChange('h6')} style={{ marginBottom: '5px', display: 'block' }} />
                                  <Checkbox label="body (body tags)" checked={selectedElements.body} onChange={() => handleElementChange('body')} style={{ marginBottom: '5px', display: 'block' }}/>
                                  <Checkbox label="Paragraph (p tags)" checked={selectedElements.p} onChange={() => handleElementChange('p')} style={{ marginBottom: '5px', display: 'block' }}/>
                                  <Checkbox label="Anchor link (a tags)" checked={selectedElements.a} onChange={() => handleElementChange('a')} style={{ marginBottom: '5px', display: 'block' }}/>
                                  <Checkbox label="List (li tags)" checked={selectedElements.li} onChange={() => handleElementChange('li')} style={{ marginBottom: '5px', display: 'block' }}/>
                              </div>
                        </Card>
                    </Layout.Section>
                    ...
                    <Layout.Section>
                        <Card title="Save" >
                           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', }}>
                                <div style={{flex:1}}></div>
                                  <Button
                                     onClick={selected === 0 ? handleSave : () => handleCreateUpdateSelectfont("google",selectedFont)}
                                    primary
                                    loading={loading}
                                    variant="primary"
                                >Save
